...
define_model <- function(p, d, q, P, D, Q, seasonal_per = seasonal_per){
  ...
  models_def <- grid_arima %>% 
    ...
    mutate(model_arima = pmap(list(p,d,q,P,D,Q, seasonal_per), define_model)) %>% 
...
models_fitted <- models_components %>% 
  ...
  mutate(fit = map2(workflow, split, ~ tryCatch(.x %>% fit(data = training(.y)),
...
test <- models_fitted %>% 
  mutate(
    modeltime_tbl = map(fit, ~ modeltime_table(.x)),
    model_calib = map2(modeltime_tbl, split,
                       ~ modeltime_calibrate(.x, testing(.y))), 
    resid = map(model_calib, 
                ~ pull(.x, .calibration_data) %>% 
                  extract2(1) %>% 
                  select(.residuals))
  ) %>% 
